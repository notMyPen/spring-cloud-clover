...
public class SecondPreFilter extends ZuulFilter {
	...
	public Object run() throws ZuulException {
		...
		if (null == a) {
			...
			ctx.setResponseBody("{\"status\":500,\"message\":\"a参数为空！\"}");//ResponseBody是自定义的(后续用JsonResult转换成的字符串)
			//logic-is-success保存于上下文，作为同类型下游Filter的执行开关（关联ThirdPreFilter，若没有传入a，ThirdPreFilter也就没有必要执行了）
