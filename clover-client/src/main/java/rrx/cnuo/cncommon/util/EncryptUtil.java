...
public class EncryptUtil {
    ...
    public static String MD5(String toBeSigned) {
        if (StringUtils.isBlank(toBeSigned)) return "";
        try {
            MessageDigest md5 = MessageDigest.getInstance("MD5");
            byte[] md5SignBytes = md5.digest(toBeSigned.getBytes("utf-8"));
            System.out.println( md5SignBytes.length );
    ...
    public static String sha1(String toBeSigned) {
        if (StringUtils.isBlank(toBeSigned)) {
            return "";
        }
    ...
    public static String aes_encrypt(String sKey, String ivParameter, String sSrc) throws Exception {
        if (StringUtils.isBlank(sKey) || StringUtils.isBlank(ivParameter) || StringUtils.isBlank(sSrc) )
    ...
    public static String aes_encrypt(String sKey, String sSrc) throws Exception {
        if (StringUtils.isBlank(sKey) || StringUtils.isBlank(sSrc) )
            throw new Exception("param is required");
     ...
     * aes解密
     * @author xuhongyu
     * @param sKey aes加密算法秘钥
     * @param ivParameter ticket的0到16位
     * @param sSrc ticket的16位之后
     * @return
     * @throws Exception
     */
    ...
    public static String aes_decrypt(String sKey, String sSrcInHex) throws Exception {
        if (StringUtils.isBlank(sKey ) || StringUtils.isBlank(sSrcInHex))
            throw new Exception("param is required");
        Cipher cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");
 ...
 // 解密
    public static String aes_decrypt_PKCS7(byte[] sKey, byte[] sSrc) throws Exception {
        ...
        String originalString = new String(original,"utf-8");
        return originalString;
//    	return aes_decrypt(sKey, sKey, sSrc);
    }
    ...
    public static String encryptUsingPublicKey(String base64PubKey, String toBeSigned) {
    	if (StringUtils.isBlank(toBeSigned) || StringUtils.isBlank(base64PubKey)) {
			return "";
		}
    	initialize();
    	try {
...
//            System.out.println( byteArrayToHexString(cipherText) );
    ...
    public static String decryptUsingPrivateKey(String base64PriKey, String toBeDecryptBase64) {
    	if (StringUtils.isBlank(toBeDecryptBase64) || StringUtils.isBlank( base64PriKey)) {
			return "";
		}
    	initialize();
    	try {
