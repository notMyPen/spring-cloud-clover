...
//		userPassport.setAvatarUrl(wxUserInfoModel.getAvatarUrl());
	...
	private JsonResult<UserInitOauthVo> getUserInitOauthVo(String openId,Long uid) throws Exception{
        ...
        redis.set(Const.REDIS_PREFIX.TICKET_FILE + ticket, uid, UserConst.TICKET_EXPIRE);
        ...
        userWxInfo.setExpireTime(UserConst.TICKET_EXPIRE);
	...
	private JsonResult<WxMiniUserInfoModel> wxOauth(OauthParam oauthParam) throws Exception{
        ...
        JSONObject tokenJson = AccessToken.getWxSessionJsonByCode(oauthParam.getCode(),weChatMiniConfigBean);
        ...
        if (jsonObject == null || !jsonObject.containsKey("watermark")
                ...
                || !weChatMiniConfigBean.getMiniAppId().equals(jsonObject.getJSONObject("watermark").getString("appid"))) {
        	return JsonResult.fail(JsonResult.AUTHORIZE_ERROR, "微信授权错误");
        }
	...
	public void updateAvatarUrlToAliOss(String rawData,Long uid) throws Exception{
		...
		if(StringUtils.isNotBlank(avatarUrl)) {
			String avatarKey = AliUtil.uploadFileFromNetUrl(avatarUrl,FileType.PIC.getType(), false, redis, aliOssConfigBean);
			UserPassport userPassport = new UserPassport();
	...
	@Override
	public JsonResult<MyPassportVo> getPassportInfo(Long uid) throws Exception {
		MyPassportVo resultVo = new MyPassportVo();
	...
	@Override
	public void updateUserCardNum(Long uid) throws Exception {
		// 翻牌人的剩余礼券次数更新：trade_buycard + card_award_record - board_turn
		int buycardCnt = orderFeignService.getBuycardCnt(uid);
		int awardCardCnt = bizFeignService.getAwardCardCnt(uid);
		int cardUserCnt = bizFeignService.getCardUserCnt(uid);
		int leftCardCnt = buycardCnt + awardCardCnt - cardUserCnt;
		
		UserPassport param = new UserPassport();
		param.setUid(uid);
		param.setCardNum(leftCardCnt);
